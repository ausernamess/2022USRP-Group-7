...
\input{header.tex}
\input{./ListStyle.tex}
\author{Chia-Cheng, Hao \quad Le-Rong, Hsu \quad Wei-Chieh, Hung \quad Shi-Hong, Su \quad TBD \quad TBD}
...
Let $\Omega$ be our sample space, which is a collection of all possible configuration of the system. Then, the idea of energy of a configuration $\omega$ can be introduced as Hamiltonian $H(\omega)$, and $\beta$ is a parameter related to temperature.
...
If $\Omega$ is a finite set, the finiteness of $\mathbb{Z}_\beta$ depend on $\beta$.
...
The property we are looking for is the subadditivity of $\lambda_n$. This property gives the limit $\mu \coloneqq \lim_n \lambda_n^{\frac{1}{n}}$. The \textbf{connective constant} $\mu$ is hard to compute, but once acquired, we can define critical beta $\beta_c \coloneqq \ln{(\mu)}$.
...
\end{enumerate}
\subsection*{Su: Critical Probability for Bond Percolation on Triangular Lattice}
\begin{thm}
The critical probability for bond percolation on triangular lattice is $2\sin (\frac{\pi}{18})$.
Proof:	
\begin{enumerate}
	\item consider a defined graph $G'$ and try to consider the relations between $G$ and $G'$.
