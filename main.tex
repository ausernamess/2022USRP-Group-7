...
\begin{enumerate}
	...
	\colorbox{pink}{
		\begin{minipage}{1 \textwidth}
			...
			$$
	 			\frac{\partial}{\partial p} \mathbb{P}_p(A)  = \mathbb{E}_p(N(A))
			$$
		\end{minipage}
	}  
	\bigskip
	...
	\item[\SOL]
		...
		\begin{equation*}
			...
			\end{aligned}
		\end{equation*} 
		Then, if we consider $F = \vmathbb{1}$ 
\end{enumerate}
...
\begin{enumerate}
	...
	\item[\SOL] TBD
	\item[\textbf{Exercise}] For a finite edge set and \textbf{any} function $F : \{0,1\}^E \to \{0,1\}$, use a direct computation to show that 
	 $$
	 \frac{\partial}{\partial p} f(p)  = \frac{1}{p(1-p)} \sum_{e \in E} \text{Cov}(F,\omega_e),\qquad f(p) \coloneqq \mathbb{E}_p [F]
	 $$ 
\end{enumerate}
