...
\theoremstyle{definition} %Le-Rong
\newtheorem*{remark}{Remark} %Le-Rong
...
\end{enumerate}
\begin{remark}
In fact, the proof holds for infinite locally finite amenable graph with some modification. 
\end{remark}
\begin{proof}
The
\end{proof}
...
\begin{enumerate}
	\item[\textbf{Remark}] For the case $p < p_c$, we can think of the event $0 \longleftarrow \partial \Lambda_{nL}$. If the previous event happens, it implies that there must exists a path from $0$ to $\partial \Lambda_{(n-1)L}$. This is shown in Fig. \ref{Exp1}.However, the inequalities may not holds.
	\begin{figure}[h]
		\centering
