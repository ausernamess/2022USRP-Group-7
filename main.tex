...
\end{flushleft}
\begin{enumerate}
	\item[\textbf{Exercise 1}] Any event $A$ a can be "approximated" by a sequence  of events that depend only on finitely many edges.  
		i.e. we can find $(A_n)_{n \geq 1}$ s.t $A_n$ only depends on edges in $[-m_n,m_n]^d$ and $\mathbb{P}(A\triangle A_n) \to 0$ ($A\triangle A_n$ is the symmetric difference of the two sets.)
