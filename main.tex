...
\begin{enumerate}
	...
	\begin{remark}
	In coupling, usually we do not want independence, so that we can compute values between random variables.
	\end{remark}
...
\begin{enumerate}
	\item[\textbf{Remark}] For the case $p < p_c$, we can think of the event $0 \longleftarrow \partial \Lambda_{nL}$. If the previous event happens, it implies that there must exsists a path from $0$ to $\partial \Lambda_{(n-1)L}$. This is shown in Fig. \ref{Exp1}. However, the inequalities may not hold because the direction of the inequality is not correct if we consider FKG.
	\begin{figure}[h]
		\centering
	...
	\end{figure}
	\item[\textbf{Exercise}] (Similar to exercise 30 in \cite{Copin18}) Can the decay rate be faster than exponential?\\
...
\begin{enumerate}
	...
	\item[\SOL] In this exercise we should use the fact that the \textbf{exponential decay} and \textbf{uniqueness} properties should work for all bond percolation on planar periodic graph $G$. Then, we may argue that $p_c(G) + P_c(G^*)$ both greater/less than equal to 1, so the sum is equal to 1.
				
	\item[\textbf{Exercise}] (Adapted from exercise 31 in \cite{Copin18}) Find $p_c$ for the bond percolation on $\mathbb{T} = \text{triangular lattice}$
	\item Consider TBD(graph). Show that \(\mathbb{P}_{P,G} \) and \(\mathbb{P}_{P^\prime ,G^\prime }\) can be couple together (in the sense that the connection properties are preserved) 
	if \(p^\prime  = 1-p\) and \(p\) satisfies \(p^3 -3p + 1 = 0\). 
	\item Using the exponential Decay to show that \(p_{c}\)  is given by the unique solution to \(p^3 -3p+1 = 0\). Deduce that \(p_c(\mathbb{T}) = 2\sin (\frac{\pi}{18})\).    
	\item[\SOL] TBD 
\end{enumerate}
...
\begin{enumerate}
	...
	\item[\SOL] 
	\begin{itemize}
		\item For the first bound, we can just think of a way to apply exponential decay. The key is that the size of the cluster gives a lower bound on how far it can reach. 
	To be more specific, if a r-radius ball has all the interior edge open, cluster with larger size must have a larger radius.
	We can conclude that \(r \geq C_d n^{\frac{1}{d}} \), where \(C_d > 0\) is a constant and \(n \in \mathbb{N}\) is the size of \(C\).   
	Hence, we can bound the probability of \(|C| \geq  n\) using the exponential decay with the event \(0 \longleftrightarrow \partial \Lambda _{n^{\frac{1}{d}}}\),
	\[
		\mathbb{P}_p(|C| \geq  n) \leq e^{\psi(p,d)n^{\frac{1}{d}}}, \qquad \text{for some } \psi(p) > 0. 
	\] 
		\item For the harder bound, we follow the hint and show that \(|C|\) has an exponential moment satisfying with the relation 
		\[
			\mathbb{E}_p[|C|] = \sum_{x \in \mathbb{Z}^d} \mathbb{P}_p(0 \longleftrightarrow x) < \infty, 	
		\]
		and 
		\[
			\mathbb{E}_p[|C|^n ] = \sum_{x_1,\dots,x_n \in \mathbb{Z}^n} \mathbb{P}_p(0 \leftrightarrow x_1 \leftrightarrow \dots \leftrightarrow x_n).
		\]
		We first have to understand one identity
		\begin{equation}\label{eq:3-2-1}
			\mathbb{P}_p(0 \longleftrightarrow x) = \sum_{0 \in S \subset \mathbb{Z}^d} \mathbb{P}_p(0 \leftrightarrow x, C = S),  
		\end{equation}
		where \(S\) is the dummy variable representing the set of connected component containing the origin. 
		Then,
		\begin{equation}
			\begin{aligned}
				\mathbb{E}_p[|C| ] &= \sum_{n=0}^{\infty} \sum_{0 \in S \subset \mathbb{Z}^d, |S| = n} n \mathbb{P}_p(C = S)  
							\\ &= \sum_{n=0}^{\infty} \sum_{0 \in S \subset \mathbb{Z}^d, |S| = n } (\sum_{x \in \mathbb{Z}^d } \mathbb{P}_p(0 \xleftrightarrow{S} x | C = S)) \mathbb{P}_p(C = S)   
							\\ &= \sum_{0 \in S \subset \mathbb{Z}^d} \sum_{x \in \mathbb{Z}^d} \mathbb{P}_p(0 \xleftrightarrow{S} x, C = S)
							\\ &= \sum_{x \in \mathbb{Z}^d}\sum_{0 \in S \subset \mathbb{Z}^d}   \mathbb{P}_p(0 \xleftrightarrow{S} x, C = S)
							\\ &=  \sum_{x \in \mathbb{Z}^d}   \mathbb{P}_p(0 \leftrightarrow x)  && \parbox[t]{5cm}{From \ref{eq:3-2-1}} 
			\end{aligned}
		\end{equation}
		On the other hand,
		\begin{equation}
			\begin{aligned}
				\mathbb{E}_p[|C|^k ] &= \sum_{n=0}^{\infty} \sum_{0 \in S \subset \mathbb{Z}^d, |S| = n} n^k \mathbb{P}_p(C = S)  
							\\ &= \sum_{n=0}^{\infty} \sum_{0 \in S \subset \mathbb{Z}^d, |S| = n } (\sum_{x \in \mathbb{Z}^d } \mathbb{P}_p(0 \xleftrightarrow{S} x | C = S))^k \mathbb{P}_p(C = S)   
							\\ &= \sum_{0 \in S \subset \mathbb{Z}^d } (\sum_{x_1,\dots,x_k \in \mathbb{Z}^d } \mathbb{P}_p(0 \xleftrightarrow{S} x_1 \xleftrightarrow{S} \dots \xleftrightarrow{S} x_k| C = S)) \mathbb{P}_p(C = S)   
							\\ &= 	\sum_{x_1,\dots,x_k \in \mathbb{Z}^d } \sum_{0 \in S \subset \mathbb{Z}^d }  \mathbb{P}_p(0 \xleftrightarrow{S} x_1 \xleftrightarrow{S} \dots \xleftrightarrow{S} x_k , C = S)
							\\ &= \sum_{x_1,\dots,x_k \in \mathbb{Z}^d }\mathbb{P}_p(0 \leftrightarrow x_1 \leftrightarrow \dots \leftrightarrow x_k )
			\end{aligned}
		\end{equation}
		TBD
	\end{itemize}	
